...
// Created an array of characters to be used
...
function generatePassword() {
  ...
  let confirmLowChar = false;
  let confirmUpChar = false;
  let confirmSpecChar = false;
  let passwordLength = 0;
  // Loop checks if password length is within parameter
  // But doesn't run again if no input or if cancelled 
  while (passwordLength < 8 || passwordLength > 128) {
    passwordLength = parseInt(prompt("Choose the length of password between 8 - 128 characters"));
    if (passwordLength < 8 || passwordLength > 128){
      alert("Length of password is either too long or too short")
    }
  }
  // Loop checks if a type of character was selected
  while (confirmNum === false && confirmLowChar === false && confirmUpChar === false && confirmSpecChar === false) {
    alert("Choose at least one type of characters!");
  ...
  // Create arrays for characters to be used
  ...
  // This then fills the password length recquirement user selected
